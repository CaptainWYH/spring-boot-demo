    ...
    public Claims parseJWT(String jwt){
        ...
        try {
            ...
            if (ObjectUtil.isNull(expire) || expire <= 0){
                throw new SecurityException(StatusEnum.TOKEN_EXPIRED);
            }
            //校验是否与redis 中的 jwt 一致，不一致代表用户已经注销/在不同的地方登录
            String redisToken = redisTemplate.opsForValue().get(redisKey);
            if (!StrUtil.equals(jwt,redisToken)){
                throw new SecurityException(StatusEnum.TOKEN_OUT_OF_CTRL);
            }
            return claims;
            ...
            throw new SecurityException(StatusEnum.TOKEN_PARSE_ERROR);
